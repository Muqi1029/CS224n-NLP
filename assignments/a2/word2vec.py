...
def negSamplingLossAndGradient(
    ...
    Loss = Jneg-sample(vc, o, U) = − log(σ(u_o @ v_c)) - sum_log(σ(−u_k @ v_c))
    """
    ...
    U = outsideVectors[indices[1:]]
    u_o = outsideVectors[outsideWordIdx]
    z = u_o @ centerWordVec
    h = U @ centerWordVec

    y_hat = sigmoid(z)  # Scaler
    loss = - np.log(y_hat) - np.sum(np.log(sigmoid(-h)))

    gradCenterVec = (y_hat - 1.0) * u_o
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = (y_hat - 1.0) * centerWordVec

    for k in range(K):
        # get the index of outside word vector from the outsideVectors
        w_k = indices[k + 1]
        u_k = outsideVectors[w_k]
        neg_y_hat = sigmoid(-u_k @ centerWordVec)
        gradOutsideVecs[w_k] += (1.0 - neg_y_hat) * centerWordVec
        gradCenterVec += (1.0 - neg_y_hat) * u_k
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outside_word in outsideWords:
        ...
        step_loss, gradCenterVec, gradOutsideVector = word2vecLossAndGradient(center_word_vec,
                                                                              outside_word_idx,
                                                                              outsideVectors,
                                                                              dataset)
        loss += step_loss
        gradCenterVecs[center_word_idx] += gradCenterVec
