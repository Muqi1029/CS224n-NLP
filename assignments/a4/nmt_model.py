...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # project the encoded h and c with 2h dimension to h, thus constructing h_dec, c_dec
        self.h_projection = None
        self.c_projection = None

        # project the encoded h with 2h dimension to h for calculating attention
        self.att_projection = None

        # project the combined vector using attention and h_dec (with 3h dimension) to h dimension to construct o_i
        self.combined_output_projection = None

        # to get the distribution of vocab
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        src_len, batch_size, embedding_size = X.shape
        # print(X.shape)
        ...
        # get the last the hidden state and cell state
        ...
        assert enc_hiddens.shape == (batch_size, src_len, self.hidden_size * 2)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens)  # this is why we change the first dimension of batch
        ...
        for Y_t in Y:
            ...
            dec_state, o_t, _ = self.step(
                Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks=enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden, dec_cell = new_dec_state  # dec_hidden: batch x hidden_size
